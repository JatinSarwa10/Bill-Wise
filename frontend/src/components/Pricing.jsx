    ...
    >
      ...
      <div className={pricingCardStyles.content}>
        ...
        <div className={pricingCardStyles.priceContainer}>
          ...
          {isAnnual && (
            <div className={pricingCardStyles.annualBadge}></div>
          )}
        </div>

        {/* Features list - You were missing this! */}
        <div className={pricingCardStyles.featuresList}>
          <ul>
...
const Pricing = () => {
  ...
  const { isSignedIn } = useAuth(); // Fixed typo: isSingnedIn → isSignedIn
  ...
  function handleCtaClick(planMeta, flags = {}) {
    if (flags.openSignInFallback || !isSignedIn) { // Fixed: isSingnedIn → isSignedIn
      if (clerk && typeof clerk.openSignIn === "function") {
        clerk.openSignIn({ redirectUrl: "/app/create-invoices" });
      } else {
        navigate("/sign-in");
      }
      return;
    ...
    navigate("/app/create-invoice", {
      state: { forPlan: planMeta?.title || null },
    });
  }
